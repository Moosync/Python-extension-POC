...
class ExtensionEventHandler:
    ...
    async def on_song_queue_changed(self, callback: Callable[[SongQueue], None]):
        return await self.add_listener("songQueueChanged", callback)
    ...
    async def on_song_changed(self, callback: Callable[[Song], None]):
        return await self.add_listener("songChanged", callback)
    ...
    async def on_song_added(self, callback: Callable[[list[Song]], None]):
        return await self.add_listener("songAdded", callback)

    async def on_song_removed(self, callback: Callable[[list[Song]], None]):
        return await self.add_listener("songRemoved", callback)

    async def on_playlist_added(self, callback: Callable[[list[Playlist]], None]):
        return await self.add_listener("playlistAdded", callback)

    async def on_playlist_removed(self, callback: Callable[[list[Playlist]], None]):
        return await self.add_listener("playlistRemoved", callback)

    async def requestedAlbumSongs(
        self,
        callback: Callable[
            [list[Album, any]], SongsWithPageTokenReturnType | ForwardRequest
        ],
    ):
        return await self.add_listener("requestedAlbumSongs", callback)
    ...
    async def requestedArtistSongs(
        ...
        return await self.add_listener("requestedArtistSongs", callback)

    async def requestedLyrics(
        self, callback: Callable[[list[Song]], str | ForwardRequest]
    ):
        return await self.add_listener("requestedLyrics", callback)

    async def requestedPlaylistFromURL(
        self,
        callback: Callable[
            [list[str, bool]], PlaylistAndSongsReturnType | ForwardRequest
        ],
    ):
    ...
    async def requestedPlaylistSongs(
        self,
        callback: Callable[
            [list[str, bool, any]], SongsWithPageTokenReturnType | ForwardRequest
        ],
    ):
        return await self.add_listener("requestedPlaylistSongs", callback)

    async def requestedPlaylists(
        self, callback: Callable[[list[bool]], PlaylistReturnType]
    ):
    ...
    async def requestedSearchResult(
        self, callback: Callable[[list[str]], SearchReturnType | ForwardRequest]
    ):
        return await self.add_listener("requestedSearchResult", callback)

    async def requestedSongFromURL(
        self, callback: Callable[[list[str, bool]], SongReturnType | ForwardRequest]
    ):
        return await self.add_listener("requestedSongFromURL", callback)

    async def customRequest(
        self, callback: Callable[[list[str]], CustomDataReturnType]
    ):
        return await self.add_listener("customRequest", callback)

    async def playbackDetailsRequested(
        self,
        callback: Callable[[list[Song]], PlaybackDetailsReturnType | ForwardRequest],
    ):
        return await self.add_listener("playbackDetailsRequested", callback)
